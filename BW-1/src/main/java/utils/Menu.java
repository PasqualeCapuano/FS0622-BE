...
//			attivo = input.equalsIgnoreCase("S");
	...
	public static void scegliMezzo() {		
		...
		List<Autobus> aut = queryAutobus.getResultList();
		List<Tram> tr = queryTram.getResultList();
		
		for(int i = 0; i < aut.size(); i++) {
			Autobus a = aut.get(i);
			System.out.println("A" + (i+1) + " - " + a.getClass().getSimpleName() + " " + a.getNumero() + ANSI_YELLOW + " direzione --> " + ANSI_RESET + a.getPartenza() + " - " + a.getCapolinea());
		}
		for(int i = 0; i < tr.size(); i++) {
			Tram t = tr.get(i);
			System.out.println("T" + (i+1) + " - " + t.getClass().getSimpleName() + " " + t.getNumero() + ANSI_YELLOW + " direzione --> " + ANSI_RESET + t.getPartenza() + " - " + t.getCapolinea());
		}
