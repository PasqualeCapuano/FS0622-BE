...
public class App {
	...
	public static void acquistaAbbonamento() {
		...
		switch (periodo) {
		case (1):
			saveAbbonamento(LocalDate.now(), LocalDate.now().plusYears(1), true, Periodicita.SETTIMANALE);
			break;
		case (2):
			saveAbbonamento(LocalDate.now(), LocalDate.now().plusYears(1), true, Periodicita.MENSILE);
			break;
		default:
			logger.error("Valore non presente nella lista!");
		}
	...
	public static Abbonamento saveAbbonamento(LocalDate dataEmissione, LocalDate dataScadenza, boolean attivo,
			Periodicita periodicita) {
		Abbonamento a = new Abbonamento(dataEmissione, dataScadenza, attivo, periodicita);
